...
class GeneralDistiller(BasicDistiller):
    ...
    def train_on_batch(self, batch, args):
        ...
        (teacher_batch, results_T), (student_batch, results_S) = get_outputs_from_batch(batch, self.t_config.device,
                                                                                        ...
                                                                                        args)
        losses_dict_all=[]
        results_T_1 = post_adaptor(self.adaptor_T(teacher_batch, results_T[0]))
        results_T_2 = post_adaptor(self.adaptor_T(teacher_batch, results_T[1]))
        results_S = post_adaptor(self.adaptor_S(student_batch, results_S))

        total_loss_1, losses_dict = self.compute_loss(results_S, results_T_1)
        total_loss_2, losses_dict = self.compute_loss(results_S, results_T_2)
